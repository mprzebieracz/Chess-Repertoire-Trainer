 ...
 */
@Entity(
    tableName = "game_stats", foreignKeys = [ForeignKey(
        entity = Game::class, parentColumns = ["id"], childColumns = ["gameId"], onDelete = ForeignKey.CASCADE
    )], indices = [Index("gameId"), Index("profileId"), Index("openingKey"), Index("color"), Index("timeCategory")]
)
