...
fun MyStatsScreen(
    ...
    Scaffold(
        topBar = { TopAppBar(title = { Text("My opening tree") }) }
    ) { padding ->
        Column(
            modifier = Modifier
                .fillMaxSize()
            ...
            if (settings.lichessUsername.isBlank() && settings.chessComUsername.isBlank()) {
                ...
                if (lichessStats != null || chessComStats != null) {
                    ...
                    if (lichessStats != null && settings.lichessUsername.isNotBlank()) {
                        Spacer(modifier = Modifier.height(8.dp))
                        PlatformStatsSection(platformLabel = "Lichess", summary = lichessStats)
                    }

                    if (chessComStats != null && settings.chessComUsername.isNotBlank()) {
                        Spacer(modifier = Modifier.height(8.dp))
...
private fun shortenOpeningKey(key: String, maxChars: Int = 40): String {
    return if (key.length <= maxChars) key else key.take(maxChars).trimEnd() + "…"
}
