...
fun OpeningTreeScreen(viewModel: OpeningTreeViewModel, onBackClick: () -> Unit) {
    ...
    ChessScreenLayout(
        ...
        topBar = {
            ChessTopBar(title = "Opening Tree", onBackClick = onBackClick)
        },
        contentBar = {
        ...
        bottomBar = {
            ChessBottomBar(
                startContent = {
                    OutlinedButton(onClick = { viewModel.onGoBack() }, enabled = uiState.canGoBack) {
                        Text("← Back")
                    }
                    Button(onClick = { viewModel.onGoRoot() }, modifier = Modifier.padding(start = 8.dp)) {
                        Text("Root")
                    }
                },
            )
