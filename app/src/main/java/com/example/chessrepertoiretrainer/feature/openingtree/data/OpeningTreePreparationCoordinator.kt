...
class OpeningTreePreparationCoordinator {
    ...
    suspend fun prepareFromStoredGames(
        profileId: Long, games: List<GameWithPgn>, color: String, timeControlFilter: String, maxGamesForTree: Int?
    ): Int {
        if (games.isEmpty()) {
        ...
        val filtered = OpeningTreeFilterUtils.filterGamesByColor(
            games = games, color = normalizedColor, timeControlFilter = normalizedTimeControl
        )
        if (filtered.isEmpty()) {
            OpeningTreeCache.clearForProfile(profileId)
            return 0
    ...
    suspend fun prepareFromFetchedGames(
        profileId: Long, games: List<FetchedGame>, color: String, timeControlFilter: String, maxGamesForTree: Int?
    ): Int {
        if (games.isEmpty()) {
            OpeningTreeCache.clearForProfile(profileId)
            return 0
        ...
        val tree = withContext(Dispatchers.Default) {
            val gamesForTree = limited.map { fetched ->
                GameForOpeningTree(
                    pgn = fetched.pgn, isUserWhite = fetched.isUserWhite, resultTag = fetched.result
                )
            }
            OpeningTreeBuilder.buildTree(gamesForTree)
        }
