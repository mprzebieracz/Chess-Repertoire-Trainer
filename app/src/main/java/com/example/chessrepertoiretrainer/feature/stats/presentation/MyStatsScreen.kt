...
fun MyStatsScreen(
    ...
    LaunchedEffect(settings.lichessUsername, settings.chessComUsername) {
        viewModel.onUserSettingsChanged(
            lichessUsername = settings.lichessUsername, chessComUsername = settings.chessComUsername
        )
    }

    Scaffold(
        topBar = { TopAppBar(title = { Text("My stats") }) }) { padding ->
        Column(
            modifier = Modifier
                .fillMaxSize()
                .padding(padding)
            ...
            if (settings.lichessUsername.isBlank() && settings.chessComUsername.isBlank()) {
                Text(
                    text = "Set your Lichess and/or Chess.com usernames in Settings to see your stats.", style = MaterialTheme.typography.bodyMedium
                )
            }
            else {
                ...
                if (lichessStats != null || chessComStats != null) {
                    Text(
                        text = "Quick stats from your games", style = MaterialTheme.typography.titleMedium
                    )
                    ...
                    Text(
                        text = "No stats yet. Download your games below to see a summary.", style = MaterialTheme.typography.bodyMedium
                    )
                }
                ...
                if (showTreeSetup) {
                    ...
                    val anyPlatformSelected = (useLichess && hasLichessAccount) || (useChessCom && hasChessComAccount)

                    Text(
                        text = "Build an opening tree from your online games", style = MaterialTheme.typography.titleMedium
                    )
                    ...
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        if (hasLichessAccount) {
                            Row(verticalAlignment = Alignment.CenterVertically) {
                                Checkbox(
                                    checked = useLichess, onCheckedChange = { useLichess = it })
                                Text(text = "Lichess")
                            }
                        }
                        if (hasChessComAccount) {
                            ...
                            Row(verticalAlignment = Alignment.CenterVertically) {
                                Checkbox(
                                    checked = useChessCom, onCheckedChange = { useChessCom = it })
                                Text(text = "Chess.com")
                            }
                        }
                    }
                    ...
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        RadioButton(
                            selected = colorFilter == "white", onClick = { colorFilter = "white" })
                        Text(text = "White")
                        Spacer(modifier = Modifier.width(16.dp))
                        RadioButton(
                            selected = colorFilter == "black", onClick = { colorFilter = "black" })
                        Text(text = "Black")
                    }
                    ...
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        Checkbox(
                            checked = bulletEnabled, onCheckedChange = { bulletEnabled = it })
                        Text(text = "Bullet")
                    }
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        Checkbox(
                            checked = blitzEnabled, onCheckedChange = { blitzEnabled = it })
                        Text(text = "Blitz")
                    }
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        Checkbox(
                            checked = rapidEnabled, onCheckedChange = { rapidEnabled = it })
                        Text(text = "Rapid")
                    }
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        Checkbox(
                            checked = classicalEnabled, onCheckedChange = { classicalEnabled = it })
                    ...
                    }, label = { Text("Use last N games (empty = all)") }, singleLine = true
                    )
                    ...
                    Button(
                        onClick = {
                            viewModel.refreshGamesForSelection(
                                useLichess = useLichess, useChessCom = useChessCom
                            )
                        }, enabled = uiState.isAnySyncing.not() && anyPlatformSelected
                    ) {
                    ...
                    if (uiState.isAnySyncing) {
                        Spacer(modifier = Modifier.height(4.dp))
                        Text(
                            text = "Syncing games and building your opening tree…", style = MaterialTheme.typography.bodySmall
                        )
                    }
                }
            }
...
private fun PlatformStatsSection(platformLabel: String, summary: GameStatsSummary) {
    Column(verticalArrangement = Arrangement.spacedBy(4.dp)) {
        Text(
            text = platformLabel, style = MaterialTheme.typography.titleSmall
        )

        Text(
            text = "Games: ${summary.totalGames} · Winrate: ${summary.winPercent}%", style = MaterialTheme.typography.bodyMedium
        )
        ...
        if (summary.byTimeCategory.isNotEmpty()) {
            ...
            if (byTimeText.isNotBlank()) {
                Text(
                    text = "By time control: $byTimeText", style = MaterialTheme.typography.bodySmall
                )
            }
        }
        ...
        if (summary.topOpenings.isNotEmpty()) {
            Text(
                text = "Top openings:", style = MaterialTheme.typography.labelMedium
            )
