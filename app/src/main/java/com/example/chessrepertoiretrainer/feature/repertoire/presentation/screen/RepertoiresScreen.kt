...
fun RepertoiresScreen(
    viewModel: RepertoiresViewModel, onNavigateToChapters: (Int) -> Unit
) {
    ...
    Card(
        ...
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                Text(
                    text = course.repertoire.name, style = MaterialTheme.typography.titleLarge
                )
                Text(
                    text = "Side: ${course.repertoire.color}", style = MaterialTheme.typography.bodyMedium
                )
                ...
                if (total > 0) {
                    ...
                    Text(
                        text = "$learned / $total lines learned ($percent%)", style = MaterialTheme.typography.bodySmall
                    )
                    LinearProgressIndicator(
                        progress = { course.learnedFraction }, modifier = Modifier
                            .fillMaxWidth()
                            .padding(top = 4.dp)
                    )
                }
                    ...
                    Text(
                        text = "No lines yet", style = MaterialTheme.typography.bodySmall
                    )
                }
            }
    ...
    AlertDialog(onDismissRequest = onDismiss, title = { Text("Add New Repertoire") }, text = {
        Column {
            ...
            Row(
                ...
                FilterChip(selected = selectedColor == "White", onClick = { selectedColor = "White" }, label = { Text("White") })
                FilterChip(selected = selectedColor == "Black", onClick = { selectedColor = "Black" }, label = { Text("Black") })
            }
        }
    }, confirmButton = {
        Button(
