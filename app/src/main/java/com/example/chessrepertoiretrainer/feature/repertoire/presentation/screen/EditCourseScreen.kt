...
fun EditCourseScreen(
                ...
                ReorderableItem(reorderState, key = chapter.id) { isDragging ->
                    ...
                    Surface(shadowElevation = elevation) {
                        if (renamingChapterId == chapter.id) {
                            RenamingChapterItem(
                                chapter = chapter,
                                onConfirm = { name -> viewModel.renameChapter(chapter, name) },
    ...
    if (showAddDialog) {
        AlertDialog(
            onDismissRequest = { showAddDialog = false; newChapterName = "" },
            ...
            dismissButton = {
                TextButton(onClick = { showAddDialog = false; newChapterName = "" }) { Text("Cancel") }
            }
        )
    }
    ...
    if (showRenameRepertoire) {
        AlertDialog(
            onDismissRequest = { showRenameRepertoire = false },
...
@Composable
private fun RenamingChapterItem(
    chapter: Chapter,
