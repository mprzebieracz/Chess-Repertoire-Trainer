...
fun SettingsScreen(viewModel: SettingsViewModel) {
    ...
    Scaffold(
        ...
        Column(
            ...
            ThemeModeSelector(
                current = settings.appThemeMode, onChange = { viewModel.updateAppThemeMode(it) })

            BoardThemeSelector(
                current = settings.boardTheme, onChange = { viewModel.updateBoardTheme(it) })
            ...
            Row(
                ...
                RadioButton(
                    selected = settings.defaultOnlinePlatform == "lichess", onClick = { viewModel.updateDefaultOnlinePlatform("lichess") })
                Text(text = "Lichess")

                Spacer(modifier = Modifier.padding(start = 16.dp))

                RadioButton(
                    selected = settings.defaultOnlinePlatform == "chess.com", onClick = { viewModel.updateDefaultOnlinePlatform("chess.com") })
                Text(text = "Chess.com")
            }
            ...
            Row(
                ...
                Column(modifier = Modifier.weight(1f)) {
                    ...
                    Text(
                        text = "Use system-derived colors on Android 12+", style = MaterialTheme.typography.bodySmall
                    )
                }
                Switch(
                    checked = settings.useDynamicColors, onCheckedChange = { viewModel.updateUseDynamicColors(it) }, colors = SwitchDefaults.colors(
...
private fun ThemeModeSelector(current: AppThemeMode, onChange: (AppThemeMode) -> Unit) {
    ...
    Row(
        ...
        Column(modifier = Modifier.weight(1f)) {
            Text("Dark theme")
            Text(
                text = "Toggle between light and dark app theme", style = MaterialTheme.typography.bodySmall
            )
        }
        Switch(
