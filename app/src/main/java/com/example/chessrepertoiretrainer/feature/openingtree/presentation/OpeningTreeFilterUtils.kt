...
import com.example.chessrepertoiretrainer.feature.openingtree.presentation.OpeningTreeFilterUtils.filterGames
...
object OpeningTreeFilterUtils {
     ...
     */
    fun filterGames(
        games: List<GameWithPgn>, colorFilter: OpeningTreeViewModel.ColorFilter, timeControlFilter: String?
    ): List<GameWithPgn> {
        ...
        if (tcRaw.isNotEmpty()) {
            val categories = tcRaw.split(',').map { it.trim().lowercase() }.filter { it.isNotEmpty() }.toSet()
