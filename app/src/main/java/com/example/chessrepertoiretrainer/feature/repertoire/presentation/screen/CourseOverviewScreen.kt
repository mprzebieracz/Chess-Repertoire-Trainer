...
fun CourseOverviewScreen(
    ...
    val repertoireId = repertoire?.id
    ...
    Scaffold(
        ...
        if (chaptersWithStats.isEmpty()) {
            Column(
                ...
                Text(
                    text = "Use the edit button to add chapters and lines.", style = MaterialTheme.typography.bodySmall
                )
            }
        }
        else {
                    ...
                    Card(
                        ...
                        Column(
                            ...
                            Row(
                                ...
                                Button(
                                    onClick = { onOpenChapterLearn(chapter.id) }, modifier = Modifier.weight(1f)
                                ) {
                                    Text("Learn")
                                }
                                OutlinedButton(
                                    onClick = { onOpenChapterReview(chapter.id) }, modifier = Modifier.weight(1f)
                                ) {
                                    Text("Review")
                                }
                                OutlinedButton(
                                    onClick = { onOpenChapterTrain(chapter.id) }, modifier = Modifier.weight(1f)
                                ) {
                                    Text("Train")
                                }
                            }
