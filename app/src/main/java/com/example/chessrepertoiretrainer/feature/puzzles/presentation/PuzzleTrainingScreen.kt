...
fun PuzzleTrainingScreen(viewModel: PuzzleTrainingViewModel, onBackClick: () -> Unit) {
    ...
    ChessScreenLayout(title = "Daily Puzzle",
                      ...
                      bottomContent = {
                          PuzzleTrainingBottomContent(uiState = uiState,
...
@Composable
private fun PuzzleTrainingTopContent(uiState: PuzzleTrainingUiState, onBackClick: () -> Unit) {
...
private fun PuzzleTrainingHeaderDetails(uiState: PuzzleTrainingUiState) {
    if (uiState.isSessionComplete && !uiState.isLoading) {
        Text(text = uiState.statusMessage ?: "Daily puzzle complete!",
    ...
    if (uiState.currentRating != null) {
        Text(text = "Rating: ${uiState.currentRating}",
             style = MaterialTheme.typography.bodyMedium,
             fontWeight = FontWeight.SemiBold)
    }
    ...
    if (uiState.attemptsForCurrent > 0) {
        Text(text = "Attempts: ${uiState.attemptsForCurrent}",
             style = MaterialTheme.typography.bodySmall)
    }
}

@Composable
private fun PuzzleTrainingBottomContent(uiState: PuzzleTrainingUiState,
                                        onShowSolution: () -> Unit,
                                        onShowHint: () -> Unit) {
    Column(modifier = Modifier
        .fillMaxWidth()
        ...
        if (!uiState.isLoading && !uiState.isSessionComplete) {
            PuzzleTrainingActionButtons(onShowSolution = onShowSolution, onShowHint = onShowHint)
        }
    }
}
...
private fun PuzzleTrainingStatusMessage(uiState: PuzzleTrainingUiState) {
    when {
        uiState.isLoading -> Text(text = "Loading puzzle...",
                                  style = MaterialTheme.typography.bodyMedium)

        uiState.isSessionComplete -> Text(text = uiState.statusMessage ?: "Daily puzzle complete!",
                                          style = MaterialTheme.typography.bodyMedium,
                                          fontWeight = FontWeight.SemiBold)

        uiState.lastMoveWasCorrect == true -> Text(text = "Correct!",
                                                   color = MaterialTheme.colorScheme.primary,
                                                   style = MaterialTheme.typography.bodyMedium)

        uiState.lastMoveWasCorrect == false -> Text(text = "Incorrect, try again",
                                                    color = MaterialTheme.colorScheme.error,
                                                    style = MaterialTheme.typography.bodyMedium)

        else -> Text(text = uiState.statusMessage ?: if (uiState.isWaitingForUserMove) {
...
@Composable
private fun PuzzleTrainingActionButtons(onShowSolution: () -> Unit, onShowHint: () -> Unit) {
